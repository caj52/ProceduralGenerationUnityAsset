...
public class Perlin 
{
	...
	private static readonly int[] permutation; 													// Doubled permutation to avoid overflow
	...
	static Perlin() 
	{
		...
		for(int x=0;x<512;x++) 
			permutation[x] = _permutation[x%256];
	}
	
	public static float Generate(double x, double y, double z)
	{
		x = Math.Abs(x);
		y = Math.Abs(y);
		z = Math.Abs(z);
